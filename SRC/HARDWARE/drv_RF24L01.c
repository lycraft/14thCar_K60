...
/******************** nrf24l01*********************************/  
const char *g_Ashining = "masihang";
uint8_t g_TxMode = 0, g_UartRxFlag = 0;
uint8_t g_UartRxBuffer[ 4 ] = { 0 };
uint8_t g_RF24L01RxBuffer[ 4 ] = { 0 }; 
uint16_t commond[4]={0};

void NRF2401_TX_RX(void)
{
uint16_t i=0;
RF24L01_Set_Mode( MODE_RX );
	i = NRF24L01_RxPacket( g_RF24L01RxBuffer );		//接收字节
		//if( 0 != i )
		{
			// for(c=0;c<i;c++)
				 {
			// commond[c]=(uint16_t)g_RF24L01RxBuffer[c];
			//OLED_Write_Num5(3, c*2, g_RF24L01RxBuffer[c]);	//输出接收到的字节
//					 OLED_Write_Num5(3, 0, g_RF24L01RxBuffer[0]);
//					 OLED_Write_Num5(3, 2, g_RF24L01RxBuffer[1]);
//					 OLED_Write_Num5(3, 4, g_RF24L01RxBuffer[2]);
//					 OLED_Write_Num5(3, 6, g_RF24L01RxBuffer[3]);
			 }
...
//	RCC_APB2PeriphClockCmd( RF24L01_CE_GPIO_CLK | RF24L01_IRQ_GPIO_CLK, ENABLE );	//?? CE IRQ????  CS SCK MISO MOSI?SPI?????
	...
	//初始化CE
	RF24L01_GpioInitStructer.GPIO_InitState = Bit_SET;
	RF24L01_GpioInitStructer.GPIO_IRQMode = GPIO_IT_DISABLE;
	RF24L01_GpioInitStructer.GPIO_Mode = GPIO_Mode_OPP;
	RF24L01_GpioInitStructer.GPIOx = RF24L01_CE_GPIO_PORT;
	RF24L01_GpioInitStructer.GPIO_Pin = RF24L01_CE;
	GPIO_Init( &RF24L01_GpioInitStructer );
	//初始化状态设置为高
	...
	RF24L01_GpioInitStructer.GPIOx = RF24L01_IRQ_GPIO_PORT;
	RF24L01_GpioInitStructer.GPIO_Pin = RF24L01_IRQ;
	GPIO_Init(&RF24L01_GpioInitStructer );
	//初始化状态设置为高
