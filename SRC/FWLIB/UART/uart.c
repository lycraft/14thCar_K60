...
void uart_putbuff1 (UART_Type* UARTx, int32 *buff, uint32_t len)
{
    while(len--)
    {
        UART_SendData1(UARTx, *buff);
        buff++;
    }
}

static void vcan_sendware(uint8_t *wareaddr,uint8_t waresize)
{
    #define CMD_WARE     3
    uint8_t cmdf[2] = {CMD_WARE, ~CMD_WARE};
    uint8_t cmdr[2] = {~CMD_WARE, CMD_WARE}; 

    uart_putbuff(UART4, cmdf, sizeof(cmdf)); 
    uart_putbuff(UART4, wareaddr, waresize); 
    uart_putbuff(UART4, cmdr, sizeof(cmdr)); 
}
