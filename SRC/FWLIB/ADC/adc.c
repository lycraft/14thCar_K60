...
uint16_t ADC_GetConversionValue(uint32_t ADCxMap)
{
	...
	ADCx->CFG1 = (0
                        ...
                        | ADC_CFG1_MODE(ADC_PRECISION_10BIT)
	...
	ADCx->SC1[0] |= pADC_Map->ADC_Chl;
	//ADC_ITConfig(ADCx, A, ADC_IT_AI, ENABLE);
	//等待转换完成
	while((ADCx->SC1[0] & ADC_SC1_COCO_MASK) == 0);
	 result = ADCx->R[0];
	//返回ADC输出返回结果
	ADCx->SC1[0] &= ~ADC_SC1_COCO_MASK;//清除标志
	return result;
}
